...
- GeoIssue's migration confirmed that PostgreSQL is the durable source of truth: local Docker PostgreSQL and the cloud API used the same relational model, while the frontend never owned the report dataset.
...
Take a restorable dump before auth/schema changes, restore it into an isolated database, compare users/reports/issues and relationships before and after migration, and only then point the deployed service at the target. Never treat a seed command or volume reset as a backup strategy.
